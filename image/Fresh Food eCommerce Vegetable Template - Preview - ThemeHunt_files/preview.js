jQuery(function($){
  ...
  $(document).on('click', '.demo-link', function(event){
    ...
    window.history.replaceState("", "", $(this).closest('.template-item').data('slug') + '/preview');
    $('.btn-purchase').attr('href', 'https://shapebootstrap.net/item/' + $(this).closest('.template-item').data('slug'))

    $('#templates-wrapper').slideUp(300);
    $('body').removeClass('popup-open');
    $('#theme-selector').removeClass('active');
    document.title = template_title + ' Preview | ThemeHunt';
    $('.theme-name').text(template_title);
    $('#demoframe > iframe').removeAttr('src').attr('src', $(this).attr('href'));
  ...
  $('#phone-landscape').on('click',function(event){
    event.preventDefault()
    $('#responsive>a').removeClass('active');
    $(this).addClass('active');
  ...
  $('#phone-portrait').on('click',function(event){
    event.preventDefault()
    $('#responsive>a').removeClass('active');
    $(this).addClass('active');
  ...
  //Controls
  $(document).on('click', '.controls>ul>li:not(".control-nav")>a', function(event) {
    event.preventDefault();
  ...
  // Previous
  $(document).on('click', '.controls>ul>li.prev>a', function(event) {
    event.preventDefault();
  ...
  $(document).on('click', '.controls>ul>li.next>a', function(event) {
    event.preventDefault();
    var prev_index = $('.controls>ul').find('>li.active').data('index');
  ...
  // iframe
  $('#demoframe > iframe').on('load', function(){
